...
public class ChessBoardToHtml implements Cloneable{
	...
	protected void drawButton(String type, String value,String name,String classe){
		drawHtmlInput(type,value,name,classe);
		this.getStreamHtml().add(cm.getContent()+ContentManagement.NEW_LINE);
	}
	
	private void drawHtmlInput(String type, String value,String name, String classe){
		html.Input in = new html.Input();
		cm.addAttribute(in, "type="+type);
		cm.addAttribute(in, "value="+value);
		cm.addAttribute(in, "class="+classe);
		cm.addAttribute(in, "name="+name);
	}
	
	protected void drawHtmlForm(String c,String action){
		html.Form fo = new html.Form();
		cm.addBetween(fo,c);
		cm.addAttribute(fo,"action="+action);
	}
	
	protected void drawBreakLine(){
		this.getStreamHtml().add("<br />"+ContentManagement.NEW_LINE);
	}
